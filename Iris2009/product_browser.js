...
AC.ProductBrowser={productSlider:null,
sliderVal:0,
animationId:false,
viewportWidth:946,
contentWidth:946,
...
hasIpDragged:false,
init:function(c){
	if(typeof(c.categories)!="undefined"){
		this.categories=c.categories
}
if(typeof(c.imageOverlap)!="undefined"){
	this.overlap=c.imageOverlap
	}
	if(typeof(c.sliderCentering)!="undefined"){
		this.sliderOffset=c.sliderCentering
}
if(typeof(c.initialCategory)!="undefined"){
	this.startIndex=0
	}
	if(typeof(c.arrowScrollAmount)!="undefined"){
		this.arrowScrollAmount=0.335
}
if(typeof(c.iPhoneCategories)!="undefined"){
	this.iPhoneCategories=c.iPhoneCategories
}
if(typeof(c.iPhoneScrollAmount)!="undefined"){
	this.iPhoneScrollAmount=c.iPhoneScrollAmount
}
if(typeof(c.iPhoneContainerWidth)!="undefined"){
	this.iPhoneContainerWidth=c.iPhoneContainerWidth
}
...
this.productSlider=new Control.Slider("pb-productsliderhandle","pb-productslidertrack",{axis:"vertical"});
if(AC.ProductBrowser.isIPhone){this.categories=this.iPhoneCategories;
...
b.id="pb-iphone-leftarrow";
var a=document.createElement("div");
a.id="pb-iphone-rightarrow";
$("productbrowser").appendChild(b);
$("productbrowser").appendChild(a);
...
this.productSlider.options.onChange=function(e){
...
var d=e+AC.ProductBrowser.throwMod;
if(d<0){
	d=0
	}
	if(d>1){
...
AC.ProductBrowser.animateSlide(d)
}
else{
	if(!AC.ProductBrowser.isSliding&&e){
		AC.ProductBrowser.isSliding=true;
AC.ProductBrowser.animateSlide(e)
}
}
};
this.productSlider.options.onSlide=function(e){
	$("pb-productsliderhandleimage").style.left=$("pb-productsliderhandle").style.left;
if(e&&!AC.ProductBrowser.isSliding){
	AC.ProductBrowser.isSliding=true;
	AC.ProductBrowser.isThrow=false;
if(AC.ProductBrowser.isMouseDown){
	AC.ProductBrowser.dif=e-AC.ProductBrowser.lastX;
AC.ProductBrowser.lastX=e;
...
var d=AC.ProductBrowser.offsetContentWidth;
$("pb-productslider").style.left=d*e+"px";
this.sliderVal=e;
AC.ProductBrowser.lastX=e;
...
Event.observe("pb-productslidertrack","mousedown",function(d){
														   var f=d.offsetX||d.layerX;
if(Event.element(d).id=="pb-productslidertrack"&&f<100){
	AC.ProductBrowser.left()
}});
...
Event.observe("pb-productsliderhandle","mouseup",function(){AC.ProductBrowser.isMouseDown=false
});
